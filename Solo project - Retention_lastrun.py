...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ritik\\Lab in Psychology\\Psychopy Experiments\\Solo Project_Sternberg task\\Solo project - Retention_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "Welcome_Screen_2" ---
    text = visual.TextStim(win=win, name='text',
        text="Welcome to the experiment!\n\nIn this experiment you will be presented with a set of numbers and after that you will be presented with a single digit and you have to press 'y' if the number if the digit was present in the set of numbers and 'n' if the number was not presented.\n\nPlease press the spacebar when you are ready!\n\nIf you have any questions, please ask!\n\nAll the best!!",
        font='Arial',
    ...
    # --- Prepare to start Routine "Welcome_Screen_2" ---
    # create an object to store info about Routine Welcome_Screen_2
    Welcome_Screen_2 = data.Routine(
        name='Welcome_Screen_2',
        components=[text, key_resp],
    )
    Welcome_Screen_2.status = NOT_STARTED
    ...
    key_resp.rt = []
    _key_resp_allKeys = []
    # store start times for Welcome_Screen_2
    Welcome_Screen_2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    Welcome_Screen_2.tStart = globalClock.getTime(format='float')
    Welcome_Screen_2.status = STARTED
    thisExp.addData('Welcome_Screen_2.started', Welcome_Screen_2.tStart)
    Welcome_Screen_2.maxDuration = None
    # keep track of which components have finished
    Welcome_Screen_2Components = Welcome_Screen_2.components
    for thisComponent in Welcome_Screen_2.components:
    ...
    # --- Run Routine "Welcome_Screen_2" ---
    Welcome_Screen_2.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                currentRoutine=Welcome_Screen_2,
            )
        ...
        if not continueRoutine:  # a component has requested a forced-end of Routine
            Welcome_Screen_2.forceEnded = routineForceEnded = True
            break
    ...
    # --- Ending Routine "Welcome_Screen_2" ---
    for thisComponent in Welcome_Screen_2.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for Welcome_Screen_2
    Welcome_Screen_2.tStop = globalClock.getTime(format='float')
    Welcome_Screen_2.tStopRefresh = tThisFlipGlobal
    thisExp.addData('Welcome_Screen_2.stopped', Welcome_Screen_2.tStop)
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "Welcome_Screen_2" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[polygon, probe_text, isi, response_text, key_resp_probe],
        )
        ...
        # Decide distractor amount
        num_distr = 10 if random.random() > 0.5 else 0
        
        # Log in data
        distractor_present = 1 if num_distr > 0 else 0
        thisExp.addData('distractor_present', distractor_present)
        ...
        for i in range(num_distr):
            ...
            # random position away from center (memory area)
            while True:
        ...
        probe_text.setText(memory_set
        
        )
        response_text.setText("Was this digit " + str(probe_digit) + " present in the numbers presented before? " 
        ...
        while continueRoutine:
            ...
            # if probe_text is starting this frame...
            if probe_text.status == NOT_STARTED and tThisFlip >= 5-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'probe_text.started')
                # update status
                probe_text.status = STARTED
            ...
            # if probe_text is stopping this frame...
            if probe_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > probe_text.tStartRefresh + 5-frameTolerance:
                    # keep track of stop time/frame for later
                    probe_text.tStop = t  # not accounting for scr refresh
                    probe_text.tStopRefresh = tThisFlipGlobal  # on global time
                    probe_text.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'probe_text.stopped')
                    # update status
                    probe_text.status = FINISHED
                    probe_text.setAutoDraw(False)
            
            # *isi* updates
            
            # if isi is starting this frame...
            if isi.status == NOT_STARTED and tThisFlip >= 10-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if response_text is starting this frame...
            if response_text.status == NOT_STARTED and tThisFlip >= 15-frameTolerance:
                # keep track of start time/frame for later
                ...
                # update status
                response_text.status = STARTED
                response_text.setAutoDraw(True)
            
            # if response_text is active this frame...
            ...
            if key_resp_probe.status == NOT_STARTED and tThisFlip >= 3.5-frameTolerance:
                # keep track of start time/frame for later
            ...
            if key_resp_probe.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_probe.getKeys(keyList=['y','n'], ignoreKeys=["escape"], waitRelease=False)
        ...
        # Run 'End Routine' code from distractor_code
        distractors = []
        ...
        trials.addData('key_resp_probe.keys',key_resp_probe.keys)
        if key_resp_probe.keys != None:  # we had a response
            trials.addData('key_resp_probe.rt', key_resp_probe.rt)
            trials.addData('key_resp_probe.duration', key_resp_probe.duration)
