...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ritik\\Lab in Psychology\\Psychopy Experiments\\Intentional Binding\\Intentional Binding_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "Welcome__screen" ---
    text_3 = visual.TextStim(win=win, name='text_3',
        text='Welcome to an Intentional Binding task!\n\nin this experiment you will be presented with a white circle which will turn green, when the circle turns green, you are requested to press the spacebar. After pressing the space bar you will hear a sound, the goal is that when you hear the sound you have to estimate the delay between the key press and the sound presentation on a scale of 0-999.',
        font='Arial',
    ...
    text = visual.TextStim(win=win, name='text',
        text='Report the delay between the key press and the beep sound on a scale (0-999). Click submit after entering the value',
        font='Arial',
    ...
    # --- Prepare to start Routine "Welcome__screen" ---
    # create an object to store info about Routine Welcome__screen
    Welcome__screen = data.Routine(
        name='Welcome__screen',
        components=[text_3],
    )
    Welcome__screen.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for Welcome__screen
    Welcome__screen.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    Welcome__screen.tStart = globalClock.getTime(format='float')
    Welcome__screen.status = STARTED
    thisExp.addData('Welcome__screen.started', Welcome__screen.tStart)
    Welcome__screen.maxDuration = None
    # keep track of which components have finished
    Welcome__screenComponents = Welcome__screen.components
    for thisComponent in Welcome__screen.components:
    ...
    # --- Run Routine "Welcome__screen" ---
    Welcome__screen.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                currentRoutine=Welcome__screen,
            )
        ...
        if not continueRoutine:  # a component has requested a forced-end of Routine
            Welcome__screen.forceEnded = routineForceEnded = True
            break
    ...
    # --- Ending Routine "Welcome__screen" ---
    for thisComponent in Welcome__screen.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for Welcome__screen
    Welcome__screen.tStop = globalClock.getTime(format='float')
    Welcome__screen.tStopRefresh = tThisFlipGlobal
    thisExp.addData('Welcome__screen.stopped', Welcome__screen.tStop)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if Welcome__screen.maxDurationReached:
        routineTimer.addTime(-Welcome__screen.maxDuration)
    elif Welcome__screen.forceEnded:
        routineTimer.reset()
    else:
    ...
    for thisTrial in trials:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        sound_beep.setSound(frequency, secs=0.3, hamming=True)
        ...
        while continueRoutine:
            ...
            if sound_beep.status == NOT_STARTED and tThisFlip >= delay-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.addData('sound_beep.started', tThisFlipGlobal)
                # update status
                sound_beep.status = STARTED
                sound_beep.play(when=win)  # sync with win flip
            
            # if sound_beep is stopping this frame...
            if sound_beep.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > sound_beep.tStartRefresh + 0.3-frameTolerance or sound_beep.isFinished:
                    # keep track of stop time/frame for later
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'sound_beep.stopped')
                    # update status
        ...
        sound_beep.pause()  # ensure sound has stopped at end of Routine
