...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ritik\\Lab in Psychology\\Psychopy Experiments\\Solo Project_Sternberg task\\Solo project_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisTrial in trials:
        ...
        num_distr = 5 if random.random() > 0.5 else 0
        letters = list(string.ascii_uppercase)
        
        distractors = []
        for i in range(num_distr):
        
            # random position away from center (memory-set area)
            while True:
        ...
        while continueRoutine:
            ...
            if probe_text.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if probe_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > probe_text.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if response_text.status == NOT_STARTED and tThisFlip >= 3.5-frameTolerance:
                # keep track of start time/frame for later
