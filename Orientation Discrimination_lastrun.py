...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ritik\\Lab in Psychology\\Psychopy Experiments\\Orientation Discrimination\\Orientation Discrimination_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('key_resp_stimuli') is None:
        # initialise key_resp_stimuli
        key_resp_stimuli = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='key_resp_stimuli',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Intro = visual.TextStim(win=win, name='Intro',
        text='Welcome to an Orientation Discrimination Experiment!\n\nIn this experiment you have two press the up arrow key if you think that the stimulus being presented is vertical then press the upward arrow and if you think it has any other direction/ tilt then press the downward arrow',
        font='Arial',
    ...
    Stimulus = visual.GratingStim(
        ...
        ori=1.0, pos=(0, 0), draggable=False, size=(0.2, 0.2), sf=7.0, phase=0.0,
        color=[1,1,1], colorSpace='rgb',
        opacity=None, contrast=0.3, blendmode='avg',
        texRes=128.0, interpolate=True, depth=-2.0)
    key_resp_stimuli = keyboard.Keyboard(deviceName='key_resp_stimuli')
    ...
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        if Intro.status == STARTED:
            ...
            if tThisFlipGlobal > Intro.tStartRefresh + 5.0-frameTolerance:
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[Fixation, Stimulus, key_resp_stimuli],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        Stimulus.setOri(tilt)
        # create starting attributes for key_resp_stimuli
        key_resp_stimuli.keys = []
        key_resp_stimuli.rt = []
        _key_resp_stimuli_allKeys = []
        ...
        while continueRoutine:
            ...
            if Stimulus.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if Stimulus.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Stimulus.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # *key_resp_stimuli* updates
            waitOnFlip = False
            
            # if key_resp_stimuli is starting this frame...
            if key_resp_stimuli.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                key_resp_stimuli.frameNStart = frameN  # exact frame index
                key_resp_stimuli.tStart = t  # local t and not account for scr refresh
                key_resp_stimuli.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(key_resp_stimuli, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'key_resp_stimuli.started')
                # update status
                key_resp_stimuli.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(key_resp_stimuli.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(key_resp_stimuli.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if key_resp_stimuli.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_stimuli.getKeys(keyList=['up','down'], ignoreKeys=["escape"], waitRelease=False)
                _key_resp_stimuli_allKeys.extend(theseKeys)
                if len(_key_resp_stimuli_allKeys):
                    key_resp_stimuli.keys = _key_resp_stimuli_allKeys[-1].name  # just the last key pressed
                    key_resp_stimuli.rt = _key_resp_stimuli_allKeys[-1].rt
                    key_resp_stimuli.duration = _key_resp_stimuli_allKeys[-1].duration
                    # was this correct?
                    if (key_resp_stimuli.keys == str(corrAns)) or (key_resp_stimuli.keys == corrAns):
                        key_resp_stimuli.corr = 1
                    else:
                        key_resp_stimuli.corr = 0
        ...
        # check responses
        if key_resp_stimuli.keys in ['', [], None]:  # No response was made
            key_resp_stimuli.keys = None
            # was no response the correct answer?!
            if str(corrAns).lower() == 'none':
               key_resp_stimuli.corr = 1;  # correct non-response
            else:
               key_resp_stimuli.corr = 0;  # failed to respond (incorrectly)
        # store data for trials (TrialHandler)
        trials.addData('key_resp_stimuli.keys',key_resp_stimuli.keys)
        trials.addData('key_resp_stimuli.corr', key_resp_stimuli.corr)
        if key_resp_stimuli.keys != None:  # we had a response
            trials.addData('key_resp_stimuli.rt', key_resp_stimuli.rt)
            trials.addData('key_resp_stimuli.duration', key_resp_stimuli.duration)
    ...
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        if Bye.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Bye.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
