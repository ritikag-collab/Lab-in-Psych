...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ritik\\Lab in Psychology\\Psychopy Experiments\\Stroop Task\\Stroop task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text="Welcome to the Stroop task\n\nYour task in this experiment is to respond to the colour of the word and not to the word itself, please respond to the ink/colour that you observed in the text.\n\nIf the colour is red please press 'r' and if it is blue please press 'b'",
        font='Arial',
    ...
    fixation_polygon = visual.ShapeStim(
        win=win, name='fixation_polygon',units='pix', 
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('Stroop_condition.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[fixation_polygon, Stroop_text, key_resp],
        )
        trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        Stroop_text.setColor(Colour, colorSpace='rgb')
        ...
        while continueRoutine:
            ...
            if Stroop_text.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Stroop_text.started')
                # update status
            ...
            # if Stroop_text is stopping this frame...
            if Stroop_text.status == STARTED:
                # is it time to stop? (based on local clock)
                if tThisFlip > 5.0-frameTolerance:
                    # keep track of stop time/frame for later
                    Stroop_text.tStop = t  # not accounting for scr refresh
                    Stroop_text.tStopRefresh = tThisFlipGlobal  # on global time
                    Stroop_text.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Stroop_text.stopped')
                    # update status
                    Stroop_text.status = FINISHED
